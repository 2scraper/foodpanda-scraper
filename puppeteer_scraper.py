#!/usr/bin/env python3
"""
foodpanda-scraper — pyppeteer edition (secondary engine)
=====================================================
...
# There is NO price floor here: a foodpanda vendor tile carries no price at
# all (0 price nodes across 1,952 tiles on two country sites), so `price` is
# not a column on this repo's `Product` and a price-coverage threshold would
# describe nothing. What IS worth a floor is the share of rows that got the
# two columns every tile publishes — 1,904 of 1,904 carried a name and an
# image. Kept byte-identical to playwright_scraper's, and the smoke suite
# asserts the three engines agree: a floor that differed between engines
# would mean one of them warning about a page its twin called healthy.
TITLE_FLOOR = 99
IMAGE_FLOOR = 95
...
# one are vendors with no reviews yet.
RATING_IS_SPARSE_BY_DESIGN = True

# A page holding less than this share of the fullest page in the same run is
# reported as thin. foodpanda batches at 48 tiles but does not guarantee it
# (48, 48 and 44 on three consecutive cold fetches), and the LAST page of a
# listing is legitimately short, so the bar stays loose.
THIN_PAGE_SHARE = 0.6
...
class PageOutcome:
    ...
    # Always None on this site: foodpanda publishes no counter of any kind
    # on a listing — no "1-48 of 1,904", no total, no ranks. Kept so the
    # sidecar's shape matches the family's, and so the absence shows up in
    # the output rather than only in a comment.
    total_available: Optional[int] = None
    # The page's own result header, verbatim, for the sidecar. Always None
    # here for the same reason.
    header: Optional[str] = None
    # The six names the page's own CollectionPage JSON-LD publishes, on a
    # page of forty-eight. Too few to build rows from; exactly right as an
    # independent cross-check of what the top of the grid holds.
...
def _driver(session):
    ...
    def scroll_to_bottom():
        ...
        # fixed distance falls behind a page that grows as it loads, and a
        # sibling repo's 2400px wheel stopped three rounds short of a 7600px
        # grid (§8). Only the HOME page is ever scrolled here.
        try:
...
def _parse_for_mode(html: str, url: str, args, page_num: int = 1) -> List:
    ...
    # `--category` deliberately does NOT reach this call: on this site the
    # `category` column holds the vendor's own cuisines, and letting a
    # command-line label write into it would mix a user's string with the
    # site's data with no way to tell them apart. The label goes to the
    # sidecar instead.
...
def _same_url(a: str, b: str) -> bool:
    ...
    strip the tracking parameters in
    product_parser.TRACKING_PARAMS (`utm_*`, click ids, `src`), so two views
    of one page never match unless both sides are cleaned. An engine with
    its own copy of this in a sibling repo got the equivalent wrong and
    silently fell back to sequential fetching.
    """
...
def _fetch_one_page(session, args, pool, page_num: int, url: str) -> PageOutcome:
    ...
    for block_attempt in range(block_retries + 1):
        ...
        if load_failed and block_attempt < block_retries:
            # Same null guard as below: a load failure with no pool is a
            # perfectly ordinary thing, and `pool.advance` on None is not.
            if has_pool:
        ...
        # No interstitial-settling step, and the reason is that foodpanda's
        # refusal does not settle into anything: PerimeterX's denial document
        # is a final answer for that session, not a page that clears itself
        ...
        if not page_flow.should_retry(state):
            ...
            # a CORRECT one — the address does not exist, and the site said
            # so with its own 404 — so retrying it would re-confirm the same
            # right answer, and rotating the exit would blame an address for
    ...
    if state == "blocked":
        ...
        # leaving the reader to buy a proxy they may not need. page_flow owns
        # that wording so all three engines say the same thing. The dump is
        # written even when empty: "0 bytes" is itself the diagnosis, and a
        # reader who finds no file cannot tell that from a run that never got
        # this far.
        ...
        logger.error(
            ...
            "own asset hosts, saved to %s. %s This is exit 3, distinct from a "
    ...
    if state == "content":
        ...
        # because foodpanda server-renders the grid: 44 tiles in the raw
        # response of one page, 44 at DOMContentLoaded, 44 ten seconds later.
        # It stays because "almost always" is not "always".
        selector = page_flow.ready_selector(args.mode)
        threshold = page_flow.min_matches(args.mode)
        # A POLL, not waitForFunction, which hands the browser a string to
        # evaluate and dies under a CSP without `unsafe-eval` (§18); see
        ...
        if found <= threshold:
            ...
            logger.info("No vendor tiles appeared in time. If this URL is "
                        "one page past the end of a listing, that is the "
        ...
        # pointing this loop at one is not harmless: it runs to 1,904 tiles
        # and 6.9 MB of DOM. See page_flow.should_scroll.
        if page_flow.should_scroll(page.url):
    ...
    if page_num == 1:
        # foodpanda publishes NO counter and no ranks, so there is no
        # arithmetic completeness check here. What the page DOES publish is
        # six vendor names in its CollectionPage JSON-LD, recorded as an
        # independent cross-check of the top of the grid.
        ...
        if outcome.jsonld_names:
            ...
            if missing:
                logger.warning(
                    ...
                    "not produce (%s) — the tile scope may have moved.",
    ...
    if products:
        ...
        if 100.0 * with_image / total < IMAGE_FLOOR:
            logger.warning(
                ...
                "measured floor of %d%%.",
                100.0 * with_image / total, page_num, IMAGE_FLOOR)
        # NO warning on the rating share: 78% on one country site and 72% on
        # another, and the tiles without one are vendors with no reviews yet.
        closed = sum(1 for p in products if p.is_open is False)
        logger.info("Page %d has %d/%d vendor(s) closed right now — a "
                    "point-in-time reading, and the most volatile column in "
                    "the schema.", page_num, closed, total)
...
def scrape(args) -> int:
                ...
                if candidates and not page_flow.pagination_agrees(
                    ...
                    if not candidates:
                        logger.info(
                            ...
                            "on this site: a COLD fetch renders no page-number "
                            "links at all, and the convention itself is "
                            "verified — a cold ?page=2 returned 48 tiles "
                            "sharing no vendor code with page 1.")
    ...
    #
    # NOT "pages x rows-per-page", which fires on healthy runs here.
    # foodpanda batches at 48 tiles but does not guarantee it: three
    # consecutive cold fetches of one listing returned 48, 48 and 44, and the
    # LAST page of any listing is legitimately short. A threshold that warns
    # on every healthy run teaches the reader to ignore it.
    #
...
def parse_args():
    p = argparse.ArgumentParser(
        description="foodpanda vendor-listing scraper (pyppeteer edition). pyppeteer is "
    ...
    p.add_argument("--mode", choices=["listing"], default="listing",
                   help="listing (the only mode): a page of vendor tiles. "
                        ...
                        "/restaurant/{code}/{slug} page could not be captured "
                        "at all (eight cold navigations and one click-through "
                        "all refused with HTTP 403, on two country sites), so "
                        "no parser was written for one.")
    p.add_argument("--category", default=None,
                   help="A label for this RUN, recorded in <out>.meta.json as "
                        "`run_label`. It deliberately does NOT write to the "
                        "`category` column, which holds the vendor's own "
                        "cuisines.")
    ...
    p.add_argument("--retries", type=int, default=3,
                   help="Attempts per page load before giving up (default 3), "
                        "each with a FRESH browser — which is what actually "
                        "clears a refusal on this site: 10 of 30 measured "
                        "navigations were served, spread across attempt "
                        "numbers. A page that comes back EMPTY is not "
                        "retried: an address that does not exist is a correct "
                        "answer, not a fault.")
