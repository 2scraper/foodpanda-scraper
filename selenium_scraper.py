#!/usr/bin/env python3
"""
foodpanda-scraper — Selenium edition (secondary engine)
====================================================
...
def _driver(session):
    ...
    def scroll_to_bottom():
        ...
        # distance falls behind a page that grows as it loads, and a sibling
        # repo's 2400px wheel stopped three rounds short of a 7600px grid
        # (§8). Only the HOME page is ever scrolled here.
        try:
...
def _same_url(a: str, b: str) -> bool:
    ...
    three engines cannot drift on it. The comparison has to survive two
    things this site actually does: a bare host redirecting to its `www.`
    form (foodpanda.sg -> www.foodpanda.sg, measured), and a page-1 URL
    written with and without an explicit `?page=1`. An engine with its own
    copy of this comparison would eventually disagree with its twins about
    whether the run had advanced.
    """
...
def _fetch_one_page(session, args, pool, page_num: int, url: str) -> PageOutcome:
    ...
    if state == "blocked":
        ...
        # written even when empty: "0 bytes" is itself the diagnosis.
        debug_html = f"{args.out}_page{page_num}_debug.html"
        with open(debug_html, "w", encoding="utf-8") as f:
            f.write(html or "")
        logger.error(
            "foodpanda did not serve this request — %d bytes, %s the site's "
            "own asset hosts, saved to %s. %s Note this engine cannot use an "
    ...
    if state == "content":
        ...
        # the other two cannot — a CSP without `unsafe-eval` refuses an
        # evaluated string outright (§18).
        ...
        if page_flow.should_scroll(d["current_url"]()):
...
def parse_args():
    p = argparse.ArgumentParser(
        description="foodpanda vendor-listing scraper (Selenium edition). Cannot authenticate a "
